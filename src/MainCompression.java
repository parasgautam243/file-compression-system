...
public class MainCompression {
    ...
    private static byte[] createZip(byte[] bytes) {
        MinPriorityQueue<ByteFrequency> nodes = getByteNodes(bytes);
        ByteFrequency root = createHuffmanTree(nodes);
        Map<Byte, String> huffmanCodes = getHuffCodes(root);
        byte[] huffmanCodeBytes = zipBytesWithCodes(bytes, huffmanCodes);
        return huffmanCodeBytes;
    }

    private static MinPriorityQueue<ByteFrequency> getByteNodes(byte[] bytes) {
        MinPriorityQueue<ByteFrequency> nodes = new MinPriorityQueue<ByteFrequency>();
        ...
        for (Map.Entry<Byte, Integer> entry : tempMap.entrySet())
            nodes.add(new ByteFrequency(entry.getKey(), entry.getValue()));
        return nodes;
    }

    private static ByteFrequency createHuffmanTree(MinPriorityQueue<ByteFrequency> nodes) {
        while (nodes.len() > 1) {
            ByteFrequency left = nodes.poll();
            ByteFrequency right = nodes.poll();
            ByteFrequency parent = new ByteFrequency(null, left.frequency + right.frequency);
            parent.left = left;
            parent.right = right;
            nodes.add(parent);
        }
    ...
    private static Map<Byte, String> getHuffCodes(ByteFrequency root) {
    ...
    private static void getHuffCodes(ByteFrequency node, String code, StringBuilder sb1) {
